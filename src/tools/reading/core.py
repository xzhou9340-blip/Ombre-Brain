...
关键行为：
- 后端地址每次调用现读：READING_API_BASE > web/reading_bridge 内嵌子进程的
  内部环回地址 http://127.0.0.1:<port>/<token>（生产默认形态：read-along 是
  ombre 同服务里的 node 子进程，请求不出公网）> 本地开发默认。
...
def _base_url() -> str:
    """每次调用现读，优先级：env READING_API_BASE > 内嵌子进程的内部环回地址 > 本地默认。

    生产（read-along 作为 ombre 同服务子进程）时 web/reading_bridge 会在子进程
    启动后写好 READING_API_BASE=http://127.0.0.1:<port>/<token>；这里的第二级
    回退覆盖「env 被清掉/尚未写入」的窗口，同样不出公网。
    """
    explicit = (os.environ.get("READING_API_BASE") or "").strip()
    if explicit:
        return explicit.rstrip("/")
    try:
        from web import reading_bridge as _bridge
        return _bridge.internal_base().rstrip("/")
    except Exception:
...
def _conn_help(exc: Exception) -> str:
    base = _base_url()
    # base 带访问 token（/<token> 路径）；/health 挂在服务根、不带 token
    root = base.split("://", 1)[-1].split("/", 1)[0]
    scheme = base.split("://", 1)[0] if "://" in base else "http"
    return (
        f"共读服务连不上（{base}）：{type(exc).__name__}: {exc}\n"
        f"排查：read-along 是 ombre 同服务里的 node 子进程——"
        f"① 看 ombre 日志里 [reading] 行（子进程启动/重启/失败原因，崩溃会自动退避重启）；"
        f"② `curl -s {scheme}://{root}/health` 应返回 {{\"ok\":true}}；"
        f"③ 子进程日志在 <DATA_DIR>/reading-server.log；"
        f"部署与环境变量见 read-along/README.md。"
    )
