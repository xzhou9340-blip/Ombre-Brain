...
关键行为：
- 启动后暴露 20 个 MCP 工具：breath/hold/grow/trace/anchor/release/
  pulse/plan/letter_write/letter_read/dream/I/peek/bark_push/
...
对外暴露：mcp/mcp_extra 两个实例 + 20 个 @mcp*.tool() 函数；HTTP 路由在 src/web/*
========================================
"""
...
async def bark_push(title: str, body: str, icon: Optional[str] = "") -> str:
    ...
    return await _with_notice(
        _t_bark.bark_push(title=title, body=body, icon=icon or ""),
        op="bark_push",
        args={
            "title_len": len(title or ""), "body_len": len(body or ""),
            "has_icon": bool(icon),
        },
    )
...
if __name__ == "__main__":
    ...
    if transport in ("sse", "streamable-http"):
        ...
        if transport == "streamable-http":
            ...
            logger.info(f"MCP 单连接器 /mcp：{len(mcp._tool_manager._tools)} 个工具统一对外暴露")
        else:
            _app = mcp.sse_app()
        _app.add_middleware(
            CORSMiddleware,
